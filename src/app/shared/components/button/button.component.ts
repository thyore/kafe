...
export class ButtonComponent implements OnInit {
  ...
  border = input<ButtonProps['border']>('none');
  ...
  baseClasses =
    'cursor-pointer font-semibold focus-visible:outline-none flex items-center justify-center focus-visible:ring-2 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50';
  ...
  shadowClasses: Record<ButtonProps['shadow'], string> = {
   ...
   borderClasses: Record<ButtonProps['border'], string> = {
    none: '',
    small: 'border hover:!bg-white/10',
    medium: 'border border-gray-300',
    large: 'border border-gray-400',
  };
  ...
  ngOnInit(): void {
    this.classes = classNames(
      ...
      this.borderClasses[this.border()],
      this.full() ? 'w-full' : '',
    );
  }
